...
function [segments,starter_list,intersection_list]=linesegmenter(image)
% This function returns individual line segments in given given image
...
% work.(for eg , the letter '0' would be in trouble.

% before segmenting image, it has to be skeletonized and spurious pixels
% have to be removed.
[starters,intersections]=starter_intersection(image); % finds starter pixels and intersection pixels in the given image.
...
if isempty(starters) % think about a perfect 0
    [r,c]=find(image==1);
    coordinates=[r,c];
...
%% Line Segmenter Main body
% This is the main loop, it would keep running until all starters are dealt
% with or minor starters are dealt with or entire skeleton has been visited.
while(isnotempty(starters_queue) || isnotempty(minor_starters_queue))%| size(visited,1)<skel_size)BE SERIOUS ABOUT THESE
    ...
    elseif (isempty(starters_queue) && isnotempty(minor_starters_queue))
        ...
        if size(unvisited_neighbours,1)>2
            minor_starters_queue=del_pixel(currentminor,minor_starters_queue);
            continue;
            %error('current minor have more than two unvisited neighbours');
        end
        if size(unvisited_neighbours,1)==2
    ...
    while(isnotmember(currentpixel,visited))
        ...
        if(isnotmember(currentpixel,intersections) && isnotmember(currentpixel,starters_queue)) %isnotmember() func cheks if currentpixel is in intersections,it also implies that current pixel has only two neighbour
            ...
            % going to delete visited pixels as well as starters from
            % neighbours and find the next pixel
            temp=1;
            ...
            elseif size(neighbours,1)==2 % Remember all visited neighbours are removed.so this two are unvisited. From this, one of them will be
                ...
                % so,till this pixel and consider  pixel other than
                % intersection as a minor.else continue
...
% [1 1 1 1
%  0 1 0 0]
% in this case, the traversal will start from (1,4) and when it reaches
% (1,2), there is two neighbours (1,1) and (2,2), since the current
% direction of traversal is in (1,1), (2,2) is added to starters_list now
% (1,1) is the current pixel, from the perspective of that, neighbours are
% (1,2) and (2,2), but (1,2) is already visited and (2,2) cant be visited
% cpr=currentpixel(1);cpc=currentpixel(2);% cpr is current pixel row and
% cpc is current pixel column [1 0 1
%  0 1 0
%  0 1 0]
% in this case whatever be the starting pixel, at central pixel, the
% direction of traversal is not preserved.
