...
[segments,starter_list,intersection_list]=linesegmenter(image); % finds line segments in given images and return coordinates of each line segments
...
for i=1:N_segments
    ...
    if size(currentsegment,1)==1
        error('Single pixel line segment!');
    else
        for j=1:(size(currentsegment,1)-1)
            currentpixel=currentsegment(j,:);
            nextpixel=currentsegment(j+1,:);
            nextdirection=finddirection(currentpixel,nextpixel); % finds direction of given next pixel with respect to current pixel
            currentdirectionvector=[currentdirectionvector;nextdirection];
        end
    end
    segmentdirection{i}=currentdirectionvector;
end
%% Detection of True Lines by applying rules 
% 
...
for i=1:N_segments
    ...
    elseif numel(currentdir_segment)<3
        ...
        %error('line segment is less than 3 pixels');
...
%% Line type detection
 ...
 for i=1:N_Truelines
     ...
     if repetition~=0
%         display('two directions have occured same no.of times');
     end
...
N_intersection=size(intersection_list,1);  % no of intersection points
N_starter=size(starter_list,1);            % no of starter points (currently not used as feature vectures, but can be used!)  
...
% Intersection and starter points  normalization
V_intersection=1-((N_intersection)/10)*2;
V_starter=1-((N_starter)/10)*2;
...
% Note: filled area is removed, spurious pixels are not removed , modified
% implementation from Deelip
featurevector=[V_horizontal,V_vertical,V_rightslant,V_leftslant,norm_horizontal,norm_vertical,norm_rightslant,norm_leftslant,V_intersection];